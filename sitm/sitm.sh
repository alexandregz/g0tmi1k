...
function cleanup() { #cleanup #mode
   ...
   if [ "$1" != "remove" ]; then
      ...
      if [ "$mode" == "non" ] ; then # Else will will remove their internet access!
         if [ "$verbose" != "0" ] || [ "$diagnostics" == "true" ] || [ "$debug" == "true" ] ; then display action "Restoring: Network" ; fi
   ...
   if [ "$debug" != "true" ] || [ "$1" == "remove" ] ; then
      ...
      command=""
      if [ -e "/tmp/sitm.tmp" ] ; then command="$command /tmp/sitm.tmp" ; fi
      if [ -e "/tmp/sitm.lock" ] ; then command="$command /tmp/sitm.lock" ; fi
      if [ -e "/tmp/sitm.rb" ] ; then command="$command /tmp/sitm.rb" ; fi
      if [ -e "/tmp/sitm.rc" ] ; then command="$command /tmp/sitm.rc" ; fi
      if [ -e "/tmp/sitm.dns" ] ; then command="$command /tmp/sitm.dns" ; fi
      if [ -e "/tmp/sitm.squid" ] ; then command="$command /tmp/sitm.squid" ; fi
      if [ -e "/tmp/sitm.pl" ] ; then command="$command /tmp/sitm.pl" ; fi
      if [ -e "/tmp/sitm.dsniff" ] ; then command="$command /tmp/sitm.dsniff" ; fi
      if [ -e "/tmp/sitm.ssl" ] ; then command="$command /tmp/sitm.ssl" ; fi
      if [ -e "/tmp/sitm.445.hashes" ] ; then command="$command /tmp/sitm.445.hashes" ; fi
      if [ -e "/tmp/sitm.139.hashes" ] ; then command="$command /tmp/sitm.139.hashes" ; fi
      if [ -e "/tmp/sitm-apache-access.log" ] ; then command="$command /tmp/sitm-apache-access.log" ; fi
      if [ -e "/tmp/sitm-apache-access-ssl.log" ] ; then command="$command /tmp/sitm-apache-access-ssl.log" ; fi
      if [ -e "/tmp/sitm-apache-error.log" ] ; then command="$command /tmp/sitm-apache-error.log" ; fi
      if [ -e "/tmp/sitm-apache-error2.log" ] ; then command="$command /tmp/sitm-apache-error2.log" ; fi
...
function update() { #update
   ...
   if [ -e "/usr/bin/svn" ] ; then
      ...
      if [ "$command" != "$svn" ] ; then
         ...
         wget -q -N -P "$www/" "http://g0tmi1k.googlecode.com/svn/trunk/sitm/index.php"
         wget -q -N -P "$www/" "http://g0tmi1k.googlecode.com/svn/trunk/sitm/vnc.reg"
